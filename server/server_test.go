...
import (
	...
	"context"
	"encoding/json"
	testutils "github.com/derekAHua/irpc/_testutils"
...
func (t *Arith) ThriftMul(_ context.Context, args *testutils.ThriftArgs_, reply *testutils.ThriftReply) error {
	reply.C = args.A * args.B
	return nil
}

func (t *Arith) ConsumingOperation(_ context.Context, args *testutils.ThriftArgs_, reply *testutils.ThriftReply) error {
	reply.C = args.A * args.B
...
func TestHandleRequest(t *testing.T) {
	...
	server := New()
	_ = server.RegisterName("Arith", new(Arith), "")
...
func Test_validIP4(t *testing.T) {
	type args struct {
		ipAddress string
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := validIP4(tt.args.ipAddress); got != tt.want {
				t.Errorf("validIP4() = %v, want %v", got, tt.want)
			}
		})
	}
}
