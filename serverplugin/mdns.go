...
func (p *MDNSRegisterPlugin) initMDNS() {
	...
	server, err := zeroconf.Register(host, "_irpcServices", p.domain, p.port, []string{s}, nil)
	if err != nil {
		panic(err)
	}
	p.server = server
...
func (p *MDNSRegisterPlugin) Register(name string, _ interface{}, metadata string) (err error) {
...
func (p *MDNSRegisterPlugin) RegisterFunction(serviceName, _ string, fn interface{}, metadata string) error {
