...
type ZooKeeperRegisterPlugin struct {
	...
	// base path for irpc server, for example com/example/irpc
	BasePath string
...
func (p *ZooKeeperRegisterPlugin) Start() error {
	...
	err := p.kv.Put(p.BasePath, []byte("irpc_path"), &store.WriteOptions{IsDir: true})
	if err != nil {
...
func (p *ZooKeeperRegisterPlugin) Register(name string, _ interface{}, metadata string) (err error) {
...
func (p *ZooKeeperRegisterPlugin) RegisterFunction(serviceName, _ string, fn interface{}, metadata string) error {
